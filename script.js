...
async function init(){
  try{
    const [loaded, visuals] = await Promise.all([
      Promise.all(BOOKS.map(b => fetch(`data/${b}.json?v=8`).then(checkResponse).then(r=>r.json()))),
      fetch('data/visuals.json?v=9').then(checkResponse).then(r=>r.json())
    ]);
...
function search(q){
  ...
  $('#readingListPanel').innerHTML = hits.length ? `<div class="reading-group"><div class="search-result-heading">검색 결과 ${hits.length}개</div><div class="los-list">${hits.slice(0,80).map(h=>`<button class="los-chip" data-book="${h.bookKey}" data-reading="${escapeAttr(h.reading.id)}" data-los="${escapeAttr(h.los.id)}"><strong>${escapeHtml(h.book.bookId)} · ${escapeHtml(h.los.label||h.los.id)}</strong><span>${highlight(h.los.title,q)}</span><span class="search-snippet">${escapeHtml(h.reading.title)}</span></button>`).join('')}</div></div>` : '<div class="reading-group">검색 결과가 없어.</div>';
  bindLosButtons();
}

...
async function loadBooks(){
  const results = await Promise.allSettled(
    BOOK_FILES.map(f => fetch(f).then(r => r.ok ? r.json() : null))
  );
  results.forEach(r => {
    if (r.status === 'fulfilled' && r.value) state.books.push(r.value);
  });
...
function openLos(key, push=true){
  ...
  if (los.losEn) head.appendChild(el('p','los-en', esc(los.losEn)));
...
function buildSearchIndex(){
  ...
  state.losIndex.forEach(({book, reading, los}, key) => {
    let text = [
      los.title, los.titleKo, los.losEn,
      (los.keywords || []).join(' ')
    ].join(' ');
    (los.blocks || []).forEach(b => {
      ...
      if (b.items === undefined && b.type === 'quiz')
        text += ' ' + (b.items || []).map(q => q.q + ' ' + q.answer).join(' ');
    });
    searchIndex.push({
      key,
